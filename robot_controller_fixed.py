...
# Tempi per la segnalazione vittima (DEVONO essere sufficientemente lunghi)
PRE_SEND_WAIT = 1.0          # secondi di stop prima di inviare
POST_SEND_WAIT = 0.5         # secondi extra dopo l'invio
TOTAL_STOP_DURATION = PRE_SEND_WAIT + POST_SEND_WAIT
...
class VictimRecognizer:
    # ... (la classe VictimRecognizer rimane invariata)
...
class RescueMazeController:
    ...
    def send_victim_identification(self, event):
        """
        Invia la posizione e il tipo della vittima al Main Supervisor.
        Formato ufficiale Erebus: struct.pack("f f c", x_m, z_m, victim_byte)
        Le coordinate devono essere in METRI come float, non in centimetri come int.
        """
        if self.emitter is not None:
            try:
                # Posizione attuale del robot in metri (float)
                position = self.get_position_xy()
                x_m = float(position[0])
                z_m = float(position[1])

                # Mappa il simbolo greco al carattere corretto per il supervisor
                label = event["label"]
                if label == 'Φ':
                    victim_byte = b'H'
                elif label == 'Ψ':
                ...
                # Formato ufficiale Erebus: due float (metri) + un char
                robot_message = struct.pack("f f c", x_m, z_m, victim_byte)
                self.emitter.send(robot_message)
                print(f"[VICTIM REPORT] Inviato: ({x_m:.4f}, {z_m:.4f}) m -> '{victim_byte.decode()}'")
            except Exception as e:
                print(f"[ERROR] Invio fallito: {e}")
        else:
            print("[VICTIM REPORT] Emitter non trovato, impossibile inviare il punteggio.")

    def start_victim_reporting(self, event, now):
        """
        Rilevata vittima: fermati immediatamente, attendi PRE_SEND_WAIT secondi,
        poi invia il messaggio, attendi POST_SEND_WAIT secondi, infine riparti.
        """
        self.victim_reporting = True
        self.pending_victim = event
        self.victim_stop_until = now + TOTAL_STOP_DURATION
        self.victim_send_time = now + PRE_SEND_WAIT
        self.stop()
        print(f"[VICTIM] Victim detected. Stopping for {PRE_SEND_WAIT}s before sending, then extra {POST_SEND_WAIT}s.")

    def update_victim_reporting(self, now):
        """
        Gestisce la pausa: rimane fermo finché non è ora di inviare,
        poi invia il messaggio una sola volta e continua a fermo fino al termine.
        """
        if not self.victim_reporting:
            return False

        # Invia il messaggio al momento giusto (una sola volta)
        if now >= self.victim_send_time and self.pending_victim is not None:
            self.send_victim_identification(self.pending_victim)
            self.pending_victim = None

        # Se lo stop totale è terminato, riprendi la navigazione
        if now >= self.victim_stop_until:
            self.victim_reporting = False
            self.reset_progress_watch(now)
            print("[VICTIM] Stop period finished, resuming navigation.")
            return False
    ...
    def check_navigation_mode_switch(self, now):
        ...
        if self.nav_mode == "RIGHT_WALL" and self.start_pos_xy is not None:
            ...
            if self._is_in_start_zone():
                print("[MODE SWITCH] Returned to start. Switching to EXPLORE mode.")
    ...
    def run(self):
        while self.robot.step(TIME_STEP) != -1:
            ...
            # Rilevamento vittime (solo se non siamo già in fase di segnalazione)
            if not self.victim_reporting:
            ...
            # Gestione della pausa per segnalazione vittima
